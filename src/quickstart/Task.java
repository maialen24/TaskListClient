...
public class Task {
    ...
    void start(String serverAddress) {
        ...
        String adapterSet = serverAddress.contains("localhost:8085") ? "TASK" : "TASKADAPTER";
        ...
        Subscription sub = new Subscription("DISTINCT","task",new String[] {"izena","egoera","kont","lista"});
        sub.setRequestedSnapshot("yes");
        sub.setDataAdapter("TASKADAPTER");
            ...
            if (!mex.trim().isEmpty()) {
                try {
                    ...
                    switch (mex) {
                        ...
                        default:
                            String s = PREFIX + mex;
                            client.sendMessage(s,"task",500, sentMessageListener, queueWhileDisconnected);
    ...
    public static class SystemOutSubscriptionListener implements SubscriptionListener {
        ...
        public void onClearSnapshot(String itemName, int itemPos) {
            System.out.println("Server has cleared the current status of the task");
        }
        ...
        public void onItemUpdate(ItemUpdate update) {
            ...
            System.out.println("TASK BERRIA @  |" + update.getValue("izena") + ": " + update.getValue("egoera"));
            System.out.println("Task kopurua: " + update.getValue("kont"));

            String[] listaZereginak=update.getValue("lista").split(",");
            System.out.println("Task Lista: ");
            for(int i=0;i<listaZereginak.length;i++){
                String a=listaZereginak[i];
                a=a.replace("[","");
                a=a.replace("TASK","");
                a=a.replace("]","");
        ...
        public void onSubscription() {
            System.out.println("Now subscribed to the task item, tasks will now start coming in");
        }
        ...
        public void onUnsubscription() {
            System.out.println("Now unsubscribed from task item, no more tasks will be received");
        }
    ...
    public static class SystemOutClientMessageListener implements ClientMessageListener {
        ...
        public void onError(String originalMessage) {
            System.out.println("task \""+originalMessage+"\" was not correctly processed by the server");
        }

        @Override
        public void onProcessed(String originalMessage) {
            System.out.println("task \""+originalMessage+"\" processed correctly");
        }
    }
